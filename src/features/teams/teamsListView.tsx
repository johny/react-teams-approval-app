...
const TeamItem: React.FC<TeamProps> = ({team, users, approvers, onTeamSelect}) => {
  let usersList = users.slice(0, USERS_TO_DISPLAY).map(getUserName).join(', ')
  if (users.length > USERS_TO_DISPLAY) {
    usersList = `${usersList}...`
  }

  let approversList = approvers.slice(0, USERS_TO_DISPLAY).map(getUserName).join(', ')
...
export const TeamsListView: React.FC<TeamsListViwProps> = ({ onTeamSelect }) => {
  ...
  const renderTeamsList = () => {
    return Object.keys(teamsById).map(teamId => {
      ...
      return <TeamItem key={teamId} team={team} users={users} approvers={approvers} onTeamSelect={onTeamSelect} />
    })
  }
